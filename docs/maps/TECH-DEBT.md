...
| `modules/security/sandboxed-apps.nix` | 95-113 | `safeVrcxDaily`, `safeWindsurfDaily`, `safeVrcxDesktop`, `safeWindsurfDesktop` wrappers | **A4 — deferred (reverted)**: Electron apps (VRCX, Windsurf) fail in bubblewrap despite --no-sandbox, /dev/shm, /tmp access. VRCX crashes with ThreadPoolForeg; Windsurf fails to launch GUI. Electron apps prove difficult to containerize reliably. May need specialized Electron sandbox configuration or alternative containment approach. Wrappers disabled 2026-04-16. |
