...
| Mullvad hardcoded IP brittleness | Audit finding (maintenance risk) | Documented+Manual | `docs/POST-STABILITY.md`, `modules/security/networking.nix` | `modules/security/networking.nix` | Hardcoded 2024 IPs may become stale; maintenance process documented. Primary defense: Mullvad's built-in lockdown-mode |
| Firefox arkenfox alignment claims | Audit finding (doc drift) | Fixed | `modules/security/browser.nix`, `docs/POST-STABILITY.md` | `modules/security/browser.nix` | Daily: arkenfox-aligned (FPI disabled, ETP Strict + TCP). Paranoid: FPI enabled (security over alignment). DoH: daily only (paranoid uses VPN server DNS) |
