...
| Identifiers / machine-id / profile separation | Madaidan | Implemented+Manual | `PROJECT-STATE.md`, `docs/INSTALL-GUIDE.md` | `modules/security/impermanence.nix`, `modules/security/governance.nix` | daily: systemd-generated unique stable ID (follows systemd guidance); paranoid: Whonix shared ID `b08dfa6083e7567a1921a715000001fb` (deliberate privacy exception to blend with Whonix users, conflicts with systemd unique-id guidance); verify with `cat /etc/machine-id` |
...
| Development & packaging doctrine | Original plan | Documented | `docs/PRE-INSTALL.md` | `modules/core/base-desktop.nix`, Git HM | Not a wave-one focus |
...
| Governance assertion count drift | Audit finding (trust bug) | Fixed | `README.md`, `PROJECT-STATE.md`, `docs/PRE-INSTALL.md` | `modules/security/governance.nix` | Count corrected to 30 assertions across all docs (includes 3 new kernel hardening assertions: kexecLoadDisabled, sysrqRestrict, ioUringDisabled) |
...
| `/var/lib/systemd` + machine-id | Audit finding (resolved) | Fixed | `modules/security/impermanence.nix`, `docs/RECOVERY.md` | `modules/security/impermanence.nix` | Paranoid now uses stable Whonix shared machine-id. Both profiles persist machine-id for operational stability. No rotation issues. |
...
| Unprivileged user namespace restriction | Madaidan | Rejected | `docs/audit/SOURCE-TOPIC-LEDGER.md`, `docs/audit/FEATURE-COVERAGE-AUDIT.md` | — | **Intentionally not implemented**: Disabling `kernel.unprivileged_userns_clone` or `user.max_user_namespaces` breaks core sandboxing (bubblewrap for browsers/apps, Flatpak). Madaidan recommends disabling; this repo prioritizes functional sandboxing over userns hardening. Compensating controls: AppArmor, seccomp, capability dropping |
| Kernel lockdown mode | Madaidan | Deferred+Manual | `docs/POST-STABILITY.md` Section 17 | `modules/core/boot.nix` | **Optional POST-STABILITY hardening**: LSM preventing kernel image access. May auto-enable with Secure Boot. Not enabled by default due to NVIDIA compatibility concerns. Test after Secure Boot stable; start with `integrity` mode. See POST-STABILITY.md for enable procedure |
| WireGuard hostname endpoint DNS exposure | External review | Fixed | `PROJECT-STATE.md`, `modules/security/wireguard.nix`, `docs/POST-STABILITY.md`, `docs/RECOVERY.md` | `modules/security/wireguard.nix` | DNS exception for hostname endpoints is a standing exception (permanent rule), not time-limited bootstrap. Docs corrected to reflect this. Added `dynamicEndpointRefreshSeconds = 600` for hostname endpoints to refresh DNS resolution periodically |
| Stale Mullvad nftables fallback docs | External review | Fixed | `docs/PRE-INSTALL.md`, `docs/POST-STABILITY.md` | `modules/security/networking.nix` | Removed references to non-existent nftables fallback for daily Mullvad app mode. Daily mode relies on Mullvad's built-in lockdown-mode; no repo-level nftables enforcement |
| Agenix/secret decryption recovery | External review | Fixed | `docs/RECOVERY.md` | — | Added comprehensive recovery section for agenix/secret decryption failures: missing age identity, lost SSH host key, secret file missing/corrupted, WireGuard secret path unavailable |
| VM TLS certificate configuration | External review | Fixed | `PROJECT-STATE.md`, `modules/security/vm-isolation.nix` | `modules/security/vm-isolation.nix` | Softened TLS hardening claim to note that SPICE/VNC TLS settings require manual certificate/x509 path configuration to be effective |
| Recovery scenario validation | External review | Documented | `docs/POST-STABILITY.md`, `docs/TEST-PLAN.md`, `docs/RECOVERY.md` | — | Recovery procedures documented but not yet tested. Added verification gap in POST-STABILITY.md and test items in TEST-PLAN.md for agenix/secret decryption failures and paranoid network blocking scenarios. Requires runtime validation on actual hardware or recovery environment |
| WireGuard dynamic endpoint refresh validation | External review | Statically checked | `docs/TEST-PLAN.md`, `flake.nix` | `modules/security/wireguard.nix` | `dynamicEndpointRefreshSeconds` option is covered by existing flake check (evaluates nixos-config and paranoid-config). Runtime validation added to TEST-PLAN.md for hostname endpoint scenarios |
